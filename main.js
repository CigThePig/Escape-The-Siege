...
function canvasPosToTile(clientX,clientY){const rect=canvas.getBoundingClientRect();let x=clientX-rect.left,y=clientY-rect.top;if(state.placeMode){const {sx:psx,sy:psy}=tileToScreen(state.player.x,state.player.y);x=(x-rect.width/2)/PLACE_ZOOM+psx+tileSize/2;y=(y-rect.height/2)/PLACE_ZOOM+psy+tileSize/2}x=Math.floor(Math.max(0,Math.min(rect.width-1,x))/(rect.width/GRID_W));y=Math.floor(Math.max(0,Math.min(rect.height-1,y))/(rect.height/GRID_H));return{x,y}}
